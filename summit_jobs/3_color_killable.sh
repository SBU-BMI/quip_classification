...
if [ $CLEAN_STARTED_FILES == 1 ]; then
  if [ -d ${PATCH_PATH} ]; then
    rm ${PATCH_PATH}/*/til_color_started.txt
  fi
fi

for (( i=1; i<=${NUM_RUNS}; i++ ))
do
	TIL_JOB_ID=til_color_${i}
    TMP="tmp.lsf"
    echo "#!/bin/bash -x" > ${TMP}
