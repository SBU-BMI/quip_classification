...
def train_round(num_epochs, network, train_fn, val_fn, classn, X_train, a_train, y_train, X_test, a_test, y_test):
...
def build_network_from_ae(classn):
    ...
    mask_map = SoftThresPerc(mask_rep, perc=95.0, alpha=0.1, beta=init.Constant(0.5), tight=100.0, name="mask_map");
    enlyr    = ChInnerProdMerge(feat_map, mask_map, name="encoder");

    skipp = batch_norm(layers.Deconv2DLayer(enlyr, 256, filter_size=(3,3), stride=1, crop='same', nonlinearity=leaky_rectify));
    ...
    add_a = batch_norm(layers.Conv2DLayer(enlyr, 512, filter_size=(1,1), nonlinearity=leaky_rectify));
    add_b = batch_norm(layers.Conv2DLayer(add_a, 512, filter_size=(1,1), nonlinearity=leaky_rectify));
    add_c = batch_norm(layers.Conv2DLayer(add_b, 512, filter_size=(1,1), nonlinearity=leaky_rectify));
    add_d = batch_norm(layers.Conv2DLayer(add_c, 512, filter_size=(1,1), nonlinearity=leaky_rectify));
    add_0 = layers.Pool2DLayer(add_d, pool_size=(25,25), stride=25, mode='average_inc_pad');
    add_1 = batch_norm(layers.DenseLayer(add_0, 256, nonlinearity=leaky_rectify));

    add_2 = batch_norm(layers.DenseLayer(gllyr, 256, nonlinearity=leaky_rectify));
    add_3 = batch_norm(layers.DenseLayer(add_2, 256, nonlinearity=leaky_rectify));
    add_4 = batch_norm(layers.DenseLayer(add_3, 128, nonlinearity=leaky_rectify));
...
def split_validation(classn):
    ...
    train_round(12, network, new_params_train_fn, val_fn, classn, X_train, a_train, y_train, X_test, a_test, y_test);
    LearningRate.set_value(np.float32(0.10*LearningRate.get_value()));
    train_round(12, network, train_fn,            val_fn, classn, X_train, a_train, y_train, X_test, a_test, y_test);
