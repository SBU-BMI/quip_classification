...
def rotate_img(x, degree):
    ...
    rotate_angle = rotate_angle * np.pi / 360.0
    x = sktransform.rotate(x, rotate_angle, preserve_range=True).astype(np.uint8)
    return x;

def data_aug_img(img, mu, sigma, deterministic=False, idraw=-1, jdraw=-1):
    #global img_indx;
    #img_indx += 1;
    #io.imsave('/pylon5/ac3uump/shahira/debug/'+ str(img_indx) + ".png",  img);
    ...
    if not deterministic:
        ...
        img = np.clip(hed2rgb( \
                rgb2hed(img / 255.0) + np.random.uniform(-1.0, 1.0, (1, 1, 3))*adj_add \
              )*255.0, 0.0, 255.0);
    ...
    if not deterministic:
        ...
        img = np.clip((img-rgb_mean)*adj_magn + rgb_mean + np.random.uniform(-1.0, 1.0, (1, 1, 3))*adj_add, 0.0, 255.0);
    ...
    #io.imsave('/pylon5/ac3uump/shahira/debug/'+ str(img_indx) + "_HE.png",  img.astype(np.uint8));
    img = zero_centering(img);
    #io.imsave('/pylon5/ac3uump/shahira/debug/'+ str(img_indx) + "_HE2.png",  img.astype(np.uint8));
...
def zero_centering(img):
    ...
    starty = (img.shape[0] - PS)//2;
    startx = (img.shape[1] - PS)//2;
    endy = starty + PS;
    endx = startx + PS;
    if(starty < 0 or startx < 0): # in case rotated the width and height will have changed
        starty = (img.shape[1] - PS)//2;
        startx = (img.shape[0] - PS)//2;
        endy = starty + PS;
        endx = startx + PS;
